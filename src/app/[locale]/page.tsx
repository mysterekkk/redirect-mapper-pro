...
export default function LandingPage({
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Supported formats */}
      <section className="container px-4 py-16 border-t">
        <h2 className="text-3xl font-bold text-center mb-8">Supported Export Formats</h2>
        <div className="flex flex-wrap justify-center gap-3 max-w-2xl mx-auto">
          {['.htaccess', 'nginx conf', 'Cloudflare CSV', 'Generic CSV', 'JSON'].map((fmt) => (
            <span key={fmt} className="rounded-full border bg-muted px-4 py-2 text-sm font-medium">
              {fmt}
            </span>
          ))}
        </div>
      ...
      <footer className="border-t py-8">
        <div className="container px-4 flex flex-col items-center gap-2 text-center text-sm text-muted-foreground">
          <p>
            {t('footerAuthor')}{' '}
            <a
